...
class GA:
    ...
    def func(self):
        a = self.POP * self.supply_want  # a: 3darray (pop_size*21*8) 转运方案
        b = (a @ omega).reshape(self.POP.shape[0], 21)  # b: 2darray (pop_size*21) 各供应商损失数量
        c = b @ self.t_trans  # 2darray (pop_size*1) 各方案总损失值
        self.target_value = c

    def constrain(self):
        constrain1 = (np.around(self.POP.sum(axis=2), decimals=9) == 1.0).sum(axis=1) == 21
    ...
    def crossover_and_mutate(self):
        ...
        for i in range(POP1.shape[0]):
            for j in range(POP1.shape[1]):
                if (np.random.rand() < self.mutation) & (self.supply_want[j] >= 6000):
    ...
    def fit(self):
        ...
        for i in range(self.iter_time):
            ...
            if self.verbose:
                print('当前迭代{}回,耗时{:.2f}s，本轮损耗量最小值{:.4f}'.format(i + 1, end - start, min_value[-1]))
...
def t(str):  # 返回1单位某类(A/B/C)原材料能制造的产品数量
    if str == 'A':
        return 1 / 0.6
    elif str == 'B':
        return 1 / 0.66
...
def p(str):  # 返回1单位某类(A/B/C)原材料的价格（以C材料价格为单位，文中假设每平方储存成本为0.1单位价格）
    if str == 'A':
        return 1.2 + 0.1
    elif str == 'B':
        return 1.1 + 0.1
...
def one_solution(supply_wanted):
    arr = []
    for i in supply_wanted:
        if i > 6000:
            n = np.random.randint((i // 6000) + 1, 9, size=1)
            m = np.random.rand(int(n))
            m = m / m.sum()
            m = np.hstack((np.zeros(8 - n), m))
            np.random.shuffle(m)
            ...
            arr.append(m)
    arr = np.array(arr)
    if ((arr * supply_wanted.reshape(-1, 1)).sum(axis=0) <= 6000).sum() == 8:
        return arr
    else:
        one_solution(supply_wanted)
        return arr


# 读取数据
data_df = pd.read_excel('2.1_out_企业需要的供应商名单.xlsx', sheet_name=0)
data = np.array(data_df)
data2_df = pd.read_excel('8家转运商的相关数据.xlsx')
data2 = np.array(data2_df)[:, 1:]  # 读取损耗率
...
order_all_week = np.zeros(21).reshape(-1, 1)  # 收集每周订货方案
trans_all_week = np.zeros(21).reshape(-1, 1)  # 收集每周转运方案
inventory = 0  # 第0周时库存为0
for week in range(24):
    # 求解订货方案
    need = 56400 - inventory  # 计算本周需求（本周库存需求-上周库存）
    ...
    # 计算本周库存
    inventory = inventory + (best_solution @ (1 - omega).reshape(-1, 1) * t_trans.reshape(-1, 1)).sum() - 28200
